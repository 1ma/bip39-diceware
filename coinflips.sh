...
echo "Enter desired number of bits of entropy for your seed."
echo -n "The BIP-39 spec allows 128, 160, 192, 224 or 256: "
read -r ENT_LENGTH

if [[ ! $ENT_LENGTH =~ ^[0-9]{3}$ || ($ENT_LENGTH -ne 128 && $ENT_LENGTH -ne 160 && $ENT_LENGTH -ne 192 && $ENT_LENGTH -ne 224 && $ENT_LENGTH -ne 256) ]];
then
...
echo
echo "Do $ENT_LENGTH coin flips and enter them one by one using 0 or 1. Have fun."
echo

ENT_BITS=""
COUNTER=1

while [ "$COUNTER" -le "$ENT_LENGTH" ]
do
  echo -n "Enter bit $COUNTER/$ENT_LENGTH: "
  read -r BIT
...
ENT_CS="$ENT_BITS$CS_BITS"
MNEMONIC_LENGTH=$(((ENT_LENGTH+CS_LENGTH)/11))
MNEMONIC=()
COUNTER=1

while [ "$COUNTER" -le "$MNEMONIC_LENGTH" ]
do
  WORD_INDEX=$(echo -n "$ENT_CS" | cut -c1-11 | cat <(echo -n "ibase=2;") - | bc)
  MNEMONIC+=("${BIP39_LIST[$WORD_INDEX]}")
  ENT_CS=$(echo -n "$ENT_CS" | cut -c12-)
  COUNTER=$((COUNTER+1))
done
...
echo "ENT+CS ($((ENT_LENGTH+CS_LENGTH)) bits):"
echo "$ENT_BITS$CS_BITS"
echo
echo "Your ${MNEMONIC_LENGTH} word mnemonic is:"
echo
echo "    ${MNEMONIC[*]}"
echo
