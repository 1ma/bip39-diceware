...
echo
echo "Select $((MNEMONIC_LENGTH-1)) random words from the diceware lists and type in each one. Select the words using the procedure described in the document."
ENT_BITS=""
COUNTER=1
while [ "$COUNTER" -lt "$MNEMONIC_LENGTH" ]
do
  echo -n "Enter word $COUNTER/$((MNEMONIC_LENGTH-1)): "
  read -r WORD

  NUMBER=$(word_index "$WORD")
  if [[ $NUMBER -eq -1 ]];
  then
...
echo
echo "$((MNEMONIC_LENGTH-1)) words encode $(((MNEMONIC_LENGTH-1)*11)) bits of entropy out of the $ENT_LENGTH bits required for a $MNEMONIC_LENGTH-word mnemonic.";
...
echo
echo "Your last word is '${BIP39_LIST[$LAST_NUMBER]}' (number ${LAST_NUMBER} on the BIP-39 list)"
echo
